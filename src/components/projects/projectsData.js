    let projectsData2 = [
        {
            id: 1,
            title: "Mi Portafolio",
            cardLink: "#home",
            cardDescription: "Es mi proyecto de portafolio, donde aplico diferentes tecnologías y es completamente responsive",
            cardTarget:"_self",
            modalSubtitle: "Desarrollo de mi Sitio Web",
            modalDescription:"Es mi sitio web, en donde hablo acerca de mi y doy a conocer mis proyectos y experiencias como desarrollador web. Cabe destacar que por primera vez utilice la libreria Bootstrap en forma de componentes y no solo como un listado de clases.",
            technologies: [
                'HTML5', 
                'CSS3', 
                'Javascript', 
                'React Js', 
                'React Hooks', 
                'Bootstrap' ],
            images: [
                {source:"http://drive.google.com/uc?export=view&id=11gzlgB5n9KiPKyD_3nED-uzyG-19MYj-" , detail: "World of PC Home"},
                {source:"http://drive.google.com/uc?export=view&id=1pvwIn68cLyqwXi0ViENnXy8OjdOASEht"  , detail: "World of PC Detalle de Producto"},
                {source:"http://drive.google.com/uc?export=view&id=1iIMR3ttt7MA1JQ1DjWXjFxWxuuTeo4i9" , detail: "World of PC Carro de Compras"},
                {source:"http://drive.google.com/uc?export=view&id=1iXIOfQddu8s2b4sKDnttAeCb4jXUM_Wg" , detail: "World of PC Orden de Compra"},
                {source:"http://drive.google.com/uc?export=view&id=1-_bFBEbubAtu9lUTBuNdlhC6sg5tWfh7"  , detail: "World of PC Lista de Productos"},
            ]
        },
        {
            id: 2,
            title: "World of PC",
            cardLink: "https://proyecto-react-raul-chang.netlify.app/",
            cardDescription: "Proyecto final para mi curso de React Js. El tema es un ecommerce de una tienda de Monitores de PC",
            cardTarget:"_blank",
            modalSubtitle: "Ecommerce de Monitores de PC",
            modalDescription:"Fue mi primer proyecto utilizando React JS y sus Hooks. También fue la primera vez que implementé el uso de base de datos No SQL a un proyecto (Firebase). Cuenta con muchas funciones comunes en ecommerce como el uso de un carro de compra, separación del listado de productos por categorías, la creación de una orden de compra, etc.",
            technologies: [
                'HTML5', 
                'CSS3', 
                'Javascript', 
                'React Js', 
                'React Hooks', 
                'Firebase' ],
            images: [
                {source:"http://drive.google.com/uc?export=view&id=11gzlgB5n9KiPKyD_3nED-uzyG-19MYj-" , detail: "World of PC Home"},
                {source:"http://drive.google.com/uc?export=view&id=1pvwIn68cLyqwXi0ViENnXy8OjdOASEht"  , detail: "World of PC Detalle de Producto"},
                {source:"http://drive.google.com/uc?export=view&id=1iIMR3ttt7MA1JQ1DjWXjFxWxuuTeo4i9" , detail: "World of PC Carro de Compras"},
                {source:"http://drive.google.com/uc?export=view&id=1iXIOfQddu8s2b4sKDnttAeCb4jXUM_Wg" , detail: "World of PC Orden de Compra"},
                {source:"http://drive.google.com/uc?export=view&id=1-_bFBEbubAtu9lUTBuNdlhC6sg5tWfh7"  , detail: "World of PC Lista de Productos"},
            ]
        },
        {
            id: 3,
            title: "xChocostorex",
            cardLink: "https://raul-chang.github.io/xChocostorex-Raul_Chang/",
            cardDescription: "Consiste en un modelo de ecommerce relacionada con chocolate. Fue un proyecto para un curso de Javascript",
            cardTarget:"_blank",
            modalsubtitle: "Ecommerce de Tienda de Chocolates",
            modalDescription:"Proyecto inicial con el cual comence mi formación en el área de desarrollo web, ",
            technologies: [
                'HTML5', 
                'CSS3', 
                'Javascript', 
            ], 
            images: [
                {source:"http://drive.google.com/uc?export=view&id=10eLfF3U5Tx3_zBHSpt1q1MabDU-uV9mx" , detail: "xChocostorex Home"},
                {source:"http://drive.google.com/uc?export=view&id=1G-nAsz-eKf9vo4527hNVV42tsfGrR1xt", detail: "xChocostorex Listado de Productos"},
                {source:"http://drive.google.com/uc?export=view&id=1pro0dxN0Vw7Yla6qeCxdoOy-tDGhOV-0" , detail: "xChocostorex Sucursales"},
                {source:"http://drive.google.com/uc?export=view&id=1dbNaMzRd2Bn1Z8AalQdUxXSldkMfFfBL" , detail: "xChocostorex Contactos"},
                {source:"http://drive.google.com/uc?export=view&id=1y5XRLZUkv9LHHP2L2HqJgtWEvm4UVe-A" , detail: "xChocostorex Envios"},
            ]            
        }
    ]

export default projectsData2


